...
const STRAPI_URL =
  "https://efficient-nature-699917a2c3.strapiapp.com/api/blog-posts";
...
/**
 * Busca todos os posts (ISR)
 */
export async function getPosts(): Promise<Post[]> {
  try {
    const res = await fetch(API_URL, {
      next: { revalidate: 120 }, // revalida a cada 2 minutos
    });
...
/**
 * Busca um post pelo slug (ISR)
 */
export async function getPostBySlug(slug: string): Promise<Post | null> {
  if (!slug) return null;

  const postApiUrl = `${STRAPI_URL}?filters[slug][$eq]=${encodeURIComponent(
    slug
  )}&${POPULATE_QUERY}`;
