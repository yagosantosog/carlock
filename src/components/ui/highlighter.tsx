...
> = ({ children, className = "" }) => {
  return (
    <div
      className={`relative h-full overflow-hidden p-px before:pointer-events-none before:absolute before:-left-48 before:-top-48 before:z-10 before:h-96 before:w-96 before:translate-x-[var(--mouse-x)] before:translate-y-[var(--mouse-y)] before:rounded-full before:bg-primary before:opacity-0 before:blur-[100px] before:transition-opacity before:duration-500 after:absolute after:inset-0 after:z-10 after:rounded-3xl after:opacity-0 after:transition-opacity after:duration-500 before:group-hover:opacity-20 ${className}`}
    >
      {children}
    </div>
  );
...
function hexToRgb(hex: string): number[] {
  ...
  if (hex.startsWith('hsl')) {
    // extract numbers from hsl(29 100% 52%)
    const [h, s, l] = hex.match(/\d+/g)?.map(Number) ?? [0,0,0];
